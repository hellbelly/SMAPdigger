...
void *t_SingleAlign(void *tid) {
	...
	while(1) {
		...
		if(param.stdout) cout<<a._str_align; else fout<<a._str_align;
		cerr<<"Thread #"<<*((bit16_t*)tid)<<": \t"<<cur_at-param.read_start+1<<" reads finished. "<<Cal_AllTime()<<" secs passed"<<endl;
...
void *t_PairAlign(void *tid) {
	...
	while(1) {
		...
		pthread_mutex_lock(&mutex_fout);
		if(param.stdout) cout<<a._str_align; else fout<<a._str_align;
		cerr<<"Thread #"<<*(bit16_t*)tid<<": \t"<<cur_at-param.read_start+1<<" read pairs finished. "<<Cal_AllTime()<<" secs passed"<<endl;
		pthread_mutex_unlock(&mutex_fout);		
	}
	pthread_mutex_lock(&mutex_fout);
...
void Do_Formatdb() {
	...
	cerr<<"Create seed table. "<<Cal_AllTime()<<" secs passed\n";
};
...
void Do_SingleAlign() {
	...
	while(read_a.LoadBatchReads(fin_a,gzfin_a,0)) {
		...
		a.Do_Batch(ref);
		if(param.stdout) cout<<a._str_align; else fout<<a._str_align;
		cerr<<read_a._index-param.read_start+1<<" reads finished. "<<Cal_AllTime()<<" secs passed"<<endl;
	}
	n_aligned=a.n_aligned;	
	read_time+=Cal_AllTime()-ref_time;
};
...
void Do_PairAlign() {
	...
	while(1) {
		...
		a.Do_Batch(ref);		
		if(param.stdout) cout<<a._str_align; else fout<<a._str_align;
		cerr<<read_a._index-param.read_start+1<<" read pairs finished. "<<Cal_AllTime()<<" secs passed"<<endl;
	}	
	n_aligned_pairs=a.n_aligned_pairs;
	n_aligned_a=a.n_aligned_a;
	n_aligned_b=a.n_aligned_b;
...
int check_ifile_format(string &filename, int gz_flag) {
	...
	if(gz_flag) {
		...
		if(s1[0]=='>') {cerr<<" \t(format: gzipped FASTA)\n"; return 0;} //fasta
		if(s1[0]=='@') {cerr<<" \t(format: gzipped FASTQ)\n"; return 1;} //fastq
		if(samopen(filename.c_str(), "rb", 0)!=0) {cerr<<" \t(format: BAM)\n"; return 3;} //BAM         
        if(samopen(filename.c_str(), "r", 0)!=0) {cerr<<" \t(format: SAM)\n"; return 2;} //SAM
	}	 
	else {
		ff.open(filename.c_str());
        ff>>s1; ff.getline(ch, 1000);
		if(s1[0]=='>') {cerr<<" \t(format: FASTA)\n"; return 0;} //fasta
		if(s1[0]=='@') {cerr<<" \t(format: FASTQ)\n"; return 1;} //fastq
        if(samopen(filename.c_str(), "rb", 0)!=0) {cerr<<" \t(format: BAM)\n"; return 3;} //BAM   		
        if(samopen(filename.c_str(), "r", 0)!=0) {cerr<<" \t(format: SAM)\n"; return 2;} //SAM
	}
...
void RunProcess(void) {
    ...
    if(param.max_snp_num<100) cerr<<"max number of mismatches: "<<param.max_snp_num;
    else cerr<<"max number of mismatches: read_length * "<<param.max_snp_num-100<<"% ";
    cerr<<"\tmax gap size: "<<param.gap<<endl;
    cerr<<"kmer cut-off ratio: "<<param.max_kmer_ratio<<endl;
    cerr<<"max multi-hits: "<<param.max_num_hits;
    cerr<<"\tmax Ns: "<<param.max_ns<<"\tseed size: "<<param.seed_size<<"\tindex interval: "<<param.index_interval<<endl;
    cerr<<"quality cutoff: "<<(int)param.qual_threshold<<"\tbase quality char: '"<<param.zero_qual<<"'"<<endl;
    cerr<<"min fragment size:"<<param.min_insert<<"\tmax fragemt size:"<<param.max_insert<<endl;
    cerr<<"start from read #"<<param.read_start<<"\tend at read #"<<param.read_end<<endl;
    cerr<<"additional alignment: "<<param.read_nt<<" in reads => "<<param.ref_nt<<" in reference"<<endl;
    
    if(param.pairend){
        cerr<<"mapping strand (read_1): ++,-+";
        if(param.chains) cerr<< ",+-,--";
    ...
    for(bit32_t i=0; i<param.n_adapter;i++) cerr<<"adapter sequence"<<i+1<<": "<<param.adapter[i]<<endl;

    if(param.RRBS_flag)
        cerr <<"RRBS mode. digestion site: "<<param.digest_site.substr(0,param.digest_pos)<<'-'<<param.digest_site.substr(param.digest_pos)<<endl;

	string err_msg;
	//pair-end alignment    
	if(param.pairend) {
		cerr<<"Pair-end alignment("<<param.num_procs<<" threads)\n";

		err_msg="failed to open read file #1 (check -a option): ";
        cerr<<"Input read file #1: "<<query_a_file;
        ...
        cerr<<"Input read file #2: "<<query_b_file;
		...
		if(param.stdout) cerr<<"Output: STDOUT\t (format: SAM)\n";
		else {
			cerr<<"Output file: "<<out_align_file;
			switch(param.out_sam) {
				case 0: cerr<<"\t (format: BSP)\n"; break;
				case 1: cerr<<"\t (format: SAM)\n"; break;
				case 2: cerr<<"\t (format: SAM, automatically convert to BAM)\n"; break;
			}
		...
		if(param.out_sam&&param.sam_header) {
			...
			sprintf(_ch,"@PG\tID:BSMAP_%s\n",version); _str.append(_ch);
		...
		cerr<<"Total number of aligned reads: \n"
	...
	//single-read alignment
		...
		cerr<<"Single-end alignment("<<param.num_procs<<" threads)\n";
		
        err_msg="failed to open read file (check -a option): ";
		cerr<<"Input read file: "<<query_a_file;
		param.gz_input=check_ifile(query_a_file, err_msg);
		param.input_format=check_ifile_format(query_a_file, param.gz_input);
		read_a.InitIndex(fin_a, gzfin_a, query_a_file);
...
int main(int argc, char *argv[])
{
	...
	cerr<<"Input reference file: "<<ref_file;
	...
	*/
    if(param.out_sam==2){
		...
		if(res) {
		    ...
		    cerr<<"warning: cannot get absolute path for sam2bam.sh input. alignment file remains in SAM format.\n"; 
		}
	}
    ref.ReleaseIndex();
	return 0;
