...
const unsigned int FIXELEMENT=10; // 144/16+1
...
struct gHit {
    ref_loc_t loc;
    bit16_t chr;
    char gap_size;  //positive for insert on read, negative for insert on ref, 0 for no-gap
    bit8_t gap_pos;
};
...
public:
    ...
    //cache line 3
    bit32_t max_dbseq_size, append_dbseq_size, total_ref_seq, digest_pos; // 16 byte
    string digest_site, useful_nt; // 32 byte
    string nx_nt; int num_procs; //48 byte
    bit32_t stdout; bit8_t zero_qual, qual_threshold, default_qual; //51 byte
    bit8_t read_nt, ref_nt, N_mis; //53 byte
    bit32_t max_kmer_num; 
 ...
 //int chains;   //0: forward strands only ; 1: forward and reverse strands	
    ...
    inline bit64_t XT64(register bit64_t tt) {tt-=(tt<<1)&tt&0xAAAAAAAAAAAAAAAAUL; return tt;}

    inline bit32_t XT(register bit32_t tt) {
        register bit32_t ss;
        //tt&=(~(tt<<1&tt))|0x55555555UL; //convert T to C
        tt-=tt<<1&tt&0xAAAAAAAAUL;
        tt-=tt>>2&0x33333333UL; // 4 bit transform
        ss=(tt&0xF0F0F0F0UL)>>1;
        tt-=ss-(ss>>3); //8 bit transform, ss+(ss<<3) = s*9
        //return (tt&0xFFUL)+(tt>>8&0xFFUL)*81+(tt>>16&0xFFUL)*6561+(tt>>24)*531441;
        ss=(tt&0xFF00FF00UL)>>2; 
        tt=(tt&0x00FF00FFUL)+ss+(ss>>2)+(ss>>6);
        //tt=(tt&0x00FF00FFUL)+ss+(ss<<4)+(ss<<6); //16 bit transform, ss+(ss<<4)+(ss<<6) = s*81
    ...
    inline bit64_t XC64(register bit64_t tt) {
        return ((~tt)<<1)|tt|0x5555555555555555ULL;
        //tt|=(~tt)<<1;
